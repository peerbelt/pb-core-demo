...
scp -i $ID_RSA cassandra-single-sample.tar root@`./$SERVER_IP`:/home/peerbelt
ssh -i $ID_RSA root@`./$SERVER_IP` 'sudo docker load -i /home/peerbelt/cassandra-single-sample.tar'
scp -i $ID_RSA elasticsearch-single-sample.tar root@`./$SERVER_IP`:/home/peerbelt
ssh -i $ID_RSA root@`./$SERVER_IP` 'sudo docker load -i /home/peerbelt/elasticsearch-single-sample.tar'
...
#scp -i $ID_RSA docker-compose-cassandra-elastic.yml root@`./$SERVER_IP`:/home/peerbelt
scp -i $ID_RSA docker-compose.yml root@`./$SERVER_IP`:/home/peerbelt
#ssh -i $ID_RSA root@`./$SERVER_IP` 'sudo docker-compose --file docker-compose-cassandra-elastic.yml up -d'
...
# Start Papertrail
ssh -i ../.docker/machines/cd-iad-peerbelt-$CIRCLE_BUILD_NUM/id_rsa root@`./$SERVER_IP`  'sudo chmod 755 /etc/init.d/remote_syslog'
ssh -i ../.docker/machines/cd-iad-peerbelt-$CIRCLE_BUILD_NUM/id_rsa root@`./$SERVER_IP` 'sudo service remote_syslog start'
